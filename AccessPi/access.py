...
from PyQt5.QtWidgets import QMainWindow, QApplication, QWidget, QTabWidget, QVBoxLayout, QMessageBox, QLabel, QSizePolicy, QLineEdit
from PyQt5.QtGui import QIcon, QPixmap

from shutil import copyfile


#Importing Raspberry Pi Cam
import picamera
...
from ui.addPhoto import Ui_AddPhoto
from train_person_group import *
...
def createPerson(username):
        ...
        body = {
                'name': username,
                'userData': 'He is our authorized user'
        }
        try:
                response = requests.request('POST', uri_base + '/face/v1.0/persongroups/%s/persons'% personGroupId, json=body, data=None, headers=request_headers, params=request_parameters)
                if response.status_code == 200:
                        print ('Response:')
                        parsedd = json.loads(response.text)
                        print(json.dumps(parsedd, sort_keys=True, indent=2))
                        parsedd_json = parsedd
                        print(response.reason)
                        if parsedd_json['personId']:
                                personId = parsedd_json['personId']
                                print('person')
                                print(personId)
                                return personId
                        else:
...
#body = {'url': 'https://i.enkimd.com/BIHYkP27lew1LcPoevdib7VEc34=/800x0//images/2017/03/35752a7d753eab50f667be70b98814f3.jpg'}
        ...
        try:
                ...
                print(response.text)
                if response.status_code != 200:
                        return 'Bad Detect Request'
                elif not parsed:
                        return 'No face detected'
                parsed_json = parsed[0]
                print (json.dumps(parsed, sort_keys=True, indent=2))
                if(parsed_json['faceRectangle']):
                        face_rectangle = parsed_json['faceRectangle']
                        left_face = face_rectangle['left']
                        top_face = face_rectangle['top']
                        width_face = face_rectangle['width']
                        height_face = face_rectangle['height']
                        print(left_face)
                        print(top_face)
                        print(width_face)
                        print(height_face)
...
#*********************************************************Adding a Person Face***********************************************
                        ...
                        target_face = '{},{},{},{}'.format(left_face, top_face, width_face, height_face)
                        personGroupId = "supper-project"
                        request_parameters = {
                                'userData': 'This is the face of Prithvi',
...
#body = {'url': 'https://i.enkimd.com/BIHYkP27lew1LcPoevdib7VEc34=/800x0//images/2017/03/35752a7d753eab50f667be70b98814f3.jpg'}
                        ...
                        try:
                                response = requests.request('POST', uri_base + '/face/v1.0/persongroups/%s/persons/%s/persistedFaces'% (personGroupId, personId), json=None, data=body, headers=request_headers, params=request_parameters)
                                print('Identify')
                                print(response.status_code)
                                print(response.text)
                                if response.status_code != 200:
                                        return 'Bad Identify Request'
                                print ('Response:')
                                parsed = json.loads(response.text)
    # Since the json returned in the response body of Face API is in the form of a list of 1 JSON object. We get the json object using the below command  
    #parsed_json = parsed[0]
...
#******************************Detecting a Face and Storing the face Rectangle attributes************************************
...
class Dash(Ui_Dashboard, QWidget):
        ...
        def __init__(self, callerObject):
                QWidget.__init__(self)
                self.setupUi(self)
                self.registerButton.clicked.connect(callerObject.registerUser)
                self.logoutButton.clicked.connect(callerObject.logout)

class AddUser(Ui_AddUser, QWidget):

        #This function is used to init the Average tab
        def __init__(self, callerObject):
                QWidget.__init__(self)
                self.setupUi(self)
                self.logoutButton.clicked.connect(callerObject.addUser)

#This class is used to display the image preview
class AddPhoto(Ui_AddPhoto, QWidget):
        #This function is used to init the homepage tab
        def __init__(self, callerObject):
                QWidget.__init__(self)
                self.setupUi(self)
                self.capturePhoto.clicked.connect(callerObject.addPhoto)
...
class App(QMainWindow):
        def __init__(self):
                ...
                self.setGeometry(5, 75, 1090, 650)
...
class OuterWidget(QWidget):
        ...
        def __init__(self, callerObject):
                ...
                self.imageLabel.setPixmap(QtGui.QPixmap(os.getcwd() + "/" + 'image.jpg'))
...
"a Secure Voice Assisted movie show booking website"))
        ...
        def logout(self):
                msg_type = 'logout'
                send_message(username, msg_type)
                faceIdentifyIndex = self.tabs.addTab(self.tab1, "Facial Authentication")
        ...
        def addUser(self):
                ...
                r = session.request('POST', register_url_base + 'index.php', json=None, data=register, headers=None, params=request_parameters)
                ...
                if personId:
                        ...
                        if returnString:
                                if returnString == 'Bad Detect Request':
                                        self.tab4.label.setText(_translate("AddPhoto", "*Picture is not upto the required quality. It could be due to poor ambient light "))
                                        self.tab4.label.setStyleSheet('color: red')

                                elif returnString == 'No Face Detected':
                                        self.tab4.label.setText(_translate("AddPhoto", "*No face detected. Please position the camera pointing towards your face"))
                                        self.tab4.label.setStyleSheet('color: red')
...
"a Secure Voice Assisted movie show booking website"))
                                self.tab2.label_3.setText(_translate("Dash", "Do you want to add a new user?"))
                                stringVar = 'You have successfully added %s as a user'%(new_user_username)
...
def threadFunction(app, camera, th_stop):
        ...
        while 1:
                ...
                if not th_stop[0].wait(0):
                        ...
                        app.windowWidget.imageLabel.setPixmap(QtGui.QPixmap(os.getcwd() + "/" + 'image.jpg'))
